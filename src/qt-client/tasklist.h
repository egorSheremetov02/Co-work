...
public:
    TaskList(QObject *par = nullptr) : QAbstractListModel(par) {
       list << Task("прjjjjоект", "28.05.2021") << Task("проект", "28.05.2021");
    };

    int rowCount(const QModelIndex &parent) const override {
        if (!parent.isValid()) return list.size();
        return 0;
    }

    QVariant data(const QModelIndex &ind, int role = Qt::DisplayRole) const override {
        if (!checkIndex(ind, CheckIndexOption::IndexIsValid)) return QVariant();
        const Task &data = list[ind.row()];
        if (role == Qt::DisplayRole) return QVariant::fromValue(data);
    ...
    bool setData(const QModelIndex &index, const QVariant &value, int role = Qt::EditRole) override {
         ...
         list[index.row()] = value.value<Task>();

...
//     auto *socket1 = new tcp::socket(get_service());
     ...
     get_socket().write_some(asio::buffer(str_auth_request.data(), str_auth_request.size()));
     std::string server_response;
     server_response.resize(1024);

     std::size_t len = get_socket().read_some(asio::mutable_buffers_1(const_cast<char *>(server_response.data()), server_response.size()));
